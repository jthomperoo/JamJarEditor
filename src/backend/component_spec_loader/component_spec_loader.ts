...
import IComponentSpecLoader from "./icomponent_spec_loader";

class ComponentSpecLoader implements IComponentSpecLoader {
    ...
    public Load(filePath: string): ComponentSpec {
        ...
        const importPath = this.convertPathToImportPath(filePath);
    ...
    private extendsComponent(source: ts.SourceFile, classDec: ts.ClassDeclaration, className: string, imports: Map<string, string>, filePath: string): boolean {
        ...
        for (const inheritor of inheritors) {
            ...
            if (inheritorImport === undefined) {
                ...
                if (path.resolve(path.dirname(filePath), inheritorImport).includes(ComponentSpecLoader.COMPONENT_CLASS_PATH)) {
                    extend = true;
                } else {
        ...
        ts.forEachChild(source, (classDec: ts.Node) => {
            ...
            for (const inheritor of inheritors) {
                ...
                if (inheritorPath === undefined) {
                    // Defined in file
                    const classConstructorResult = this.getClassConstructor(source, inheritor, filePath);
                ...
                const outsideFileSource = ts.createSourceFile(filePath, outsideFileSourceCode, ts.ScriptTarget.ES5);
                const classConstructorResult = this.getClassConstructor(outsideFileSource, inheritor, outsideFilePath);
                if (classConstructorResult === undefined) {
                    continue;
                }
                constructorDec = classConstructorResult[0];
        ...
        ts.forEachChild(parameterDec, (typeDec) => {
            ...
            if (ts.isTypeReferenceNode(typeDec)) {
                ...
                if (typePath === undefined) {
                    ...
                    const importPath = this.convertPathToImportPath(outsideFilePath);
...
export default ComponentSpecLoader;